...
import Foundation
import UIKit
class OTagListView:UIScrollView
{
    var numberOfRows = 0
    var currentRow = 0
    var tags = [UILabel]()
    ...
    func addTag(_ text:String,target:AnyObject,tapAction:Selector?,longPressAction:Selector?,backgroundColor:UIColor,textColor:UIColor, tag:Int)
    {
        //instantiate label
        ...
        label.font = UIFont(name: ToucheApp.Fonts.Light.montserrat, size: 15)
    ...
    func removeTag(_ tag:UILabel) {
        if let idx = tags.index(of: tag) {
            removeTagWithIndex(idx - 1)
        }
    }
    ...
    fileprivate func layoutTagsFromIndex(_ index:Int,animated:Bool = false)
    {
        if tags.count == 0
        {
            return
