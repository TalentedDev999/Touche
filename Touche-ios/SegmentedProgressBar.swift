...
class SegmentedProgressBar: UIView {
    ...
    // hacky way to prevent layouting again
    ...
    private func next() {
        ...
        if newIndex < self.segments.count {
            ...
            let currentSegment = self.segments[currentAnimationIndex]

            print("\(currentSegment.topSegmentView.frame.size.width) == \(currentSegment.bottomSegmentView.frame.width)")
            if currentSegment.topSegmentView.frame.size.width == currentSegment.bottomSegmentView.frame.width {
